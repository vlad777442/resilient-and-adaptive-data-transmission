...
public:
    ...
    std::pair<double, std::vector<int>> find_min_time_configuration() {
        double min_time = time_threshold;  // Start with threshold as max allowed time
        ...
        for (int i = 0; i < 17; i++) {
            ...
            // Only consider configurations that meet the time constraint
            if (E_Toverall <= time_threshold && E_Toverall < min_time) {
                min_time = E_Toverall;
                best_m = current_m;
            }
        }
...
private:
    ...
    std::vector<long long> tier_sizes;
    double t_threshold = TIME_CONSTR;  // Add time threshold member
    
    // New member variables for tier-by-tier transmission
    size_t current_tier_ = 0;
    bool waiting_for_tier_completion_ = false;
    std::vector<size_t> max_retransmissions_per_tier_;
    std::vector<size_t> retransmission_count_per_tier_;

    std::map<size_t, std::chrono::steady_clock::time_point> tier_start_times_;
    std::map<size_t, std::chrono::steady_clock::time_point> tier_end_times_;
    std::map<size_t, double> tier_durations_; // In seconds
...
public:
          ...
          tier_sizes(tier_sizes)
    {
        ...
        // Initialize retransmission tracking
    ...
    void stop_transmission() {
        ...
        // Output tier times before general statistics
    ...
    void send_tier(size_t tier_id) {
        if (tier_id >= fragments_.fragments.size()) {
            ...
            output_tier_transmission_times(); // Add this line
            send_final_eot();
            return;
        }
        ...
        waiting_for_tier_completion_ = true;
    ...
    void output_tier_transmission_times() {
        ...
        for (const auto& [tier_id, duration] : tier_durations_) {
            std::cout << "Tier " << tier_id << ": " << std::fixed << std::setprecision(3) 
                      << duration << " seconds" << std::endl;
            total_time += duration;
        }
        
        // Handle any tiers that weren't completed
        for (const auto& [tier_id, start_time] : tier_start_times_) {
            if (tier_durations_.find(tier_id) == tier_durations_.end()) {
                auto partial_duration = std::chrono::duration_cast<std::chrono::microseconds>(
                    std::chrono::steady_clock::now() - start_time);
                double partial_seconds = partial_duration.count() / 1000000.0;
                std::cout << "Tier " << tier_id << ": " << std::fixed << std::setprecision(3) 
                          << partial_seconds << " seconds (incomplete)" << std::endl;
                total_time += partial_seconds;
            }
        }
        
        std::cout << "Total transmission time: " << std::fixed << std::setprecision(3) 
    ...
    void send_metadata(FragmentStore& store) {
        ...
        for (size_t tier_id = 0; tier_id < store.fragments.size(); ++tier_id) {
            ...
            if (!chunks.empty() && !chunks[0].empty()) {
                tier_metadata->set_k(chunks[0][0].k());
            }
    ...
    void retransmit_fragment(DATA::Fragment fragment, uint32_t tier_id) {
        ...
        // ✅ Use async send instead of blocking
        udp_socket_.async_send_to(
            boost::asio::buffer(serialized_fragment),
            receiver_endpoint_,
            [this, serialized_fragment](boost::system::error_code ec, std::size_t bytes_sent) {
                if (!ec) {
...
private:
    ...
    void handle_tcp_message(const std::vector<char>& buffer) {
        ...
        if (ack.ParseFromArray(buffer.data(), buffer.size()) && ack.tier_id() >= 0) {
            ...
            // Record tier completion time and calculate duration
            tier_end_times_[ack.tier_id()] = std::chrono::steady_clock::now();
            if (tier_start_times_.find(ack.tier_id()) != tier_start_times_.end()) {
                auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
                    tier_end_times_[ack.tier_id()] - tier_start_times_[ack.tier_id()]);
                tier_durations_[ack.tier_id()] = duration.count() / 1000000.0; // Convert to seconds
                
                std::cout << "Tier " << ack.tier_id() << " completed in " 
                        << tier_durations_[ack.tier_id()] << " seconds" << std::endl;
            }
            
            // Move to the next tier
            current_tier_++;
            waiting_for_tier_completion_ = false;
            send_tier(current_tier_);
            return;
        }
    ...
    void handle_report(DATA::FragmentsReport report) {
        ...
        // Calculate remaining time based on the time threshold
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::steady_clock::now() - start_transmission_time_);
        ...
        if (remaining_time < 0) {
            ...
            // Post a task to stop transmission after a brief delay
            timer_.expires_after(std::chrono::milliseconds(100));
...
int main() {
    ...
    try {
        ...
        std::vector<size_t> max_retransmissions = {50, 50, 50, 50}; // Max retransmissions per tier
